...
class CommandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create command" do
    assert_difference('Command.count') do
      post commands_url, params: { command: { accessed_at: @command.accessed_at, text: @command.text, uuid: @command.uuid } }
    end
  ...
  test "should update command" do
    patch command_url(@command), params: { command: { accessed_at: @command.accessed_at, text: @command.text, uuid: @command.uuid } }
